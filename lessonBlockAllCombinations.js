...
window.addEventListener('load', (event) => {
  ...
  if (window.location.href.indexOf("pageid=-9") > -1) {
	...
	//This is to cater for the end of lesson page that appears if a question page is the last page in the lesson and grades are activated
	//check to see if there is text on the page to indicate the student is on the end of lesson page and store it
	var checkEndOfLesson = $("div h3:contains('Congratulations - end of lesson reached')").get()
	//check to see if the view grades link is on the page and store it
	var viewGradesLink = $(".lessonbutton:contains('View grades')").get();
	//double the height of the progress bar
	$('div.progress').css({'height':'2.5rem','line-height':'2.3rem'});
	//make progress bar with rounded edges and margin above it to separate it from text
	$('.progress').addClass('rounded mt-3');
	//change progress bar from orange to green
	$('.progress-bar').addClass('bg-success');
	//ensure text on the progress bar is on one line
	$('.progress-bar').css('display','block');
	//make it so the text on the progress bar says xxx% Complete [checkered flag icon]
	$('.progress-bar').append(' <span>Complete <i class="fa fa-fw fa-flag-checkered"></i></span>');
	//make a dark rounded border around the end of lesson text
	$('.boxaligncenter').addClass('border border-dark px-3 rounded');
	//make the automatic links on the page turn into buttons
	$('.boxaligncenter a.lessonbutton').addClass('btn btn-secondary btn-block p-2 mt-3 text-center');
	//change the name on the return to unit link to match the section title the lesson sits in
	$(returnToSectionLink).html(linkUrl);
	//change the url the return to unit link sends students to, to send them to the section the lesson sites in
	$(returnToSectionLink).attr('href', $('ol.breadcrumb li:nth-last-child(2) > a').attr('href'));
	//add the words Return to before the section name, so it says Return to [section] on the button
	$(returnToSectionLink).prepend('Return to ');
	//add an icon to the end of the return to section button
	$(returnToSectionLink).append('&nbsp;&nbsp;<i class="fa fa-reply fa-fw"></i>');
	//This is to cater for the end of lesson page that appears if a question page is the last page in the lesson
	//check that the url is this exact url and check to see if the text on the page confirms the student has reached the end of lesson page
	if (window.location.href=='https://lms.monash.edu/mod/lesson/view.php' && $(checkEndOfLesson).text() == 'Congratulations - end of lesson reached'){
		//add icon to view grades button
		$(viewGradesLink).append('&nbsp;&nbsp;<i class="fa fa-check-square-o fa-fw"></i>');
		//in this case, if there are 2 buttons, they are return to unit and view grades
		if(numberOfButtons == 2){
			//add text to indicate end of lesson and what user can do on this screen
			$('.boxaligncenter div').first().before('<h5 class="mb-3">You have completed the activity <i class="fa fa-thumbs-o-up" aria-hidden="true"></i>, Where to next?</h5><ul style="color:black;" id="returnToSectionDescription"><li>Return to the section this lesson is part of <i class="fa fa-reply" aria-hidden="true"></i></li><li>Or view your grades for this activity <i class="fa fa-check-square-o fa-fw" aria-hidden="true"></i></li></ul>')
		}
		//in this case, if there are 3 buttons, they are go to next activity (review button not present), return to unit and view grades
		else if(numberOfButtons == 3 && reviewButton.length == 0){
			//add an icon to the go to next activity button
			$('.boxaligncenter a.lessonbutton:nth-last-child(3)').append('&nbsp;&nbsp;<i class="fa fa-hand-o-right" aria-hidden="true"></i>');
			//add text to indicate end of lesson and what user can do on this screen
			$('.boxaligncenter div').first().before('<h5 class="mb-3">You have completed the activity <i class="fa fa-thumbs-o-up" aria-hidden="true"></i>, Where to next?</h5><ul style="color:black;" id="returnToSectionDescription"><li>Continue onto the next activity <i class="fa fa-hand-o-right" aria-hidden="true"></i></li><li>Or return to the section this lesson is part of <i class="fa fa-reply" aria-hidden="true"></i></li><li>Or view your grades for this activity <i class="fa fa-check-square-o fa-fw" aria-hidden="true"></i></li></ul>');
		}
		//in this case, if there are 3 buttons, they are review (review button present), return to unit and view grades
		else if (numberOfButtons == 3 && reviewButton.length == 1){
			//add an icon to the review button
			$('.boxaligncenter a.lessonbutton:nth-last-child(3)').prepend('<i class="fa fa-hand-o-left" aria-hidden="true"></i>&nbsp;&nbsp;');
			//add text to indicate end of lesson and what user can do on this screen
			$('.boxaligncenter div').first().before('<h5 class="mb-3">You have completed the activity <i class="fa fa-thumbs-o-up" aria-hidden="true"></i>, Where to next?</h5><ul style="color:black;" id="returnToSectionDescription"><li>Review this lesson before continuing <i class="fa fa-hand-o-left" aria-hidden="true"></i></li><li>Return to the section this lesson is part of <i class="fa fa-reply" aria-hidden="true"></i></li><li>Or view your grades for this activity <i class="fa fa-check-square-o fa-fw" aria-hidden="true"></i></li></ul>');
		}
		//in this case, if there are 4 buttons, all autolinks are activated they are review, go to next activity, return to unit and view grades
		else if (numberOfButtons == 4){
			//add an icon to the review button
			$('.boxaligncenter a.lessonbutton:nth-last-child(4)').prepend('<i class="fa fa-hand-o-left" aria-hidden="true"></i>&nbsp;&nbsp;');
			//add an icon to the go to next activity button
			$('.boxaligncenter a.lessonbutton:nth-last-child(3)').append('&nbsp;&nbsp;<i class="fa fa-hand-o-right" aria-hidden="true"></i>');
			//add text to indicate end of lesson and what user can do on this screen
			$('.boxaligncenter div').first().before('<h5 class="mb-3">You have completed the activity <i class="fa fa-thumbs-o-up" aria-hidden="true"></i>, Where to next?</h5><ul style="color:black;" id="returnToSectionDescription"><li>Review this lesson before continuing <i class="fa fa-hand-o-left" aria-hidden="true"></i></li><li>Or continue onto the next activity <i class="fa fa-hand-o-right" aria-hidden="true"></i></li><li>Return to the section this lesson is part of <i class="fa fa-reply" aria-hidden="true"></i></li><li>Or view your grades for this activity <i class="fa fa-check-square-o fa-fw" aria-hidden="true"></i></li></ul>');
		}
	}
	else{
		//do nothing
